...
/* 
 ...
 {90, 90,  0,  0} 대기자세
 {90, 44 , 31, 0} 픽업  , P_apr{90, 50, 	25, 0}
 ...
 */
		...
		if( ( StartStop_button && city_dst ) )
		{
			...
			if(pick_regi == 2)
			{
				pos.x=j1_static; pos.y=50; pos.z=25; pos_2.pump=0; //apr pos
				iwh_move_rate.sleep();
				pick_regi++;
			}
			if(pick_regi == 4)
			{
				pos.x=j1_static; pos.y=44; pos.z=31; pos_2.pump=1; //pick pos
				iwh_move_rate.sleep();
				pick_regi++;
			}
			if(pick_regi == 6)
			{
				...
				pos.x=j1_static; pos.y=50; pos.z=25; pos_2.pump=1; //apr pos
				iwh_move_rate.sleep();
				pick_regi++;
			}			
			if(pick_regi == 8)
		...
		if( place_regi || pick_regi ){
			if( !(place_regi==999 || pick_regi==999) ){
				GuiMsg.m_state = 2;
			}
			pub_3.publish(GuiMsg);
		}else if( StartStop_button == 0 ){
			GuiMsg.m_state = 3;
			pub_3.publish(GuiMsg);
		}else{
			pub_3.publish(GuiMsg);
