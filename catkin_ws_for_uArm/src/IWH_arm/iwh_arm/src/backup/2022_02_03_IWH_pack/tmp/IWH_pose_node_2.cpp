 ...
 */
void iwh_vision2_topic_callback(const swiftpro::VClassify& msg)
{
	city_dst = msg.city;
}
...
/* 
 ...
 {90, 50 , 45, 0} 구리	, G_apr{90, 65, 20, 0}
 */
	if( 1<=city_dst && city_dst<=3 )
	{
		printf("city_dst's inputed 픽업 , pick_regi : %d \n\n",pick_regi);
		...
		if(pick_regi == 10)
		{
			...
			case 3:
				...
				if(place_regi == 1)
				{
					pos.x=j1_static; pos.y=65; pos.z=20; pos_2.pump=1; //apr pos
					iwh_move_rate.sleep();
					if(place_regi == 1) {place_regi++; break;}
				}
				if(place_regi == 2)
				{
					pos.x=j1_static; pos.y=50; pos.z=45; pos_2.pump=1; //place pos
					iwh_move_rate.sleep();
					if(place_regi == 2) {place_regi++; break;}
				}
				if(place_regi == 3)
				{
					pos.x=j1_static; pos.y=65; pos.z=20; pos_2.pump=0; //apr pos
					iwh_move_rate.sleep();
					if(place_regi == 3) {place_regi++; break;}
				}			
				if(place_regi == 4)
