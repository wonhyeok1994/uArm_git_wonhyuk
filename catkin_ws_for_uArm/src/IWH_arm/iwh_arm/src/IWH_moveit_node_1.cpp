...
//float motor_angle[3] = {0.0, 90.0, .0};   //broken init pos
float motor_angle[3] = {90.0, 125.0, 45.0}; //웅크린자세
...
{
	printf("msg.x : %f  msg.y : %f  msg.z : %f\n",msg.x,msg.y,msg.z);
	motor_angle[0] = msg.x;
	motor_angle[1] = msg.y;
	motor_angle[2] = msg.z;
}
...
int main(int argc, char **argv)
{
  printf("moveit_node.cpp START");
	...
	ros::init(argc, argv, "IWH_moveit_node_1");
	ros::NodeHandle n;
	
	ros::Subscriber sub1 = n.subscribe("iwh_ctl_topic_1", 1, iwh_ctl_topic_callback);
		
	ros::Publisher 	pub = n.advertise<swiftpro::position>("position_write_topic_1", 1);
