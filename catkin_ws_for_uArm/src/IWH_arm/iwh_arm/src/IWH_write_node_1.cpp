...
{	
	...
	ros::Subscriber sub1 = nh.subscribe("position_write_topic_1", 1, position_write_callback);	
	ros::Subscriber sub5 = nh.subscribe("iwh_state_topic_1", 1, pump_callback);

	//ros::Publisher 	 pub = nh.advertise<swiftpro::SwiftproState>("SwiftproState_topic", 1);
	ros::Publisher 	 pub = nh.advertise<std_msgs::UInt8>("A_Place_topic", 1);
	...
	while (ros::ok())
	{
		A_place_topic.data = RB_pos_regi;

		if(pub_timing == 1){
			pub_reset++;
			pub.publish(A_place_topic);
			pub_timing = 0;
